...
void* count3s_thread(void *tid)
{
  ...
  temp_arr[*id*CACHELINE_SZ/sizeof(int)] = 0;

  for(i =start; i < start+length_per_thread; i++)
  {
    if (array[i] == 3){
			temp_arr[*id*CACHELINE_SZ/sizeof(int)] += 1;
    }
  }
  pthread_mutex_lock(&lock);
  count += temp_arr[*id*CACHELINE_SZ/sizeof(int)];
...
int main (int argc, char *argv[])
{
  ...
  /* temp_arr = (int *) malloc (sizeof (int) * t); */
  temp_arr = (int *) malloc (CACHELINE_SZ * t);
